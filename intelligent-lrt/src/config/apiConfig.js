...
const IP_ADDRESSES_TO_TRY = [
  // Primary IP address
  '192.168.8.113',
  '10.194.144.155',  // New primary IP
  '0.0.0.0',  // All network interfaces
  '10.34.215.44',  // Main server IP
  '10.31.31.44',  // Main server IP
  '10.41.105.155',  // Main server IP
  ...
  '10.31.31.44',    // VirtualBox network IP
  '10.41.105.155',  // VirtualBox network IP
  
  
  // Common development IPs
  '10.12.188.44',
  '192.168.86.1',  // Local server IP
  '10.41.105.155',  // Local server IP
  '192.168.1.100',
  '10.31.31.44',
  '192.168.1.101',
  '192.168.1.102',
...
const findWorkingIp = async () => {
  ...
  // Try the new primary IP first (10.194.144.155)
  try {
    console.log(`🔍 Trying primary IP: 10.194.144.155`);
    const response = await axios.get(`http://10.194.144.155:${SERVER_PORT}/test`, { 
      timeout: 1000 
    });
    if (response.status === 200) {
      lastWorkingIp = '10.194.144.155';
      console.log(`✅ Primary IP 10.194.144.155 works!`);
      return '10.194.144.155';
    }
  } catch (_error) {
    console.log(`❌ Primary IP 10.194.144.155 failed, trying 0.0.0.0...`);
  }
  
  // Try 0.0.0.0 next
  try {
    console.log(`🔍 Trying IP: 0.0.0.0`);
    const response = await axios.get(`http://0.0.0.0:${SERVER_PORT}/test`, { 
      timeout: 1000 
    });
    if (response.status === 200) {
      lastWorkingIp = '0.0.0.0';
      console.log(`✅ IP 0.0.0.0 works!`);
      return '0.0.0.0';
    }
  } catch (_error) {
    console.log(`❌ IP 0.0.0.0 failed, trying others...`);
  }
