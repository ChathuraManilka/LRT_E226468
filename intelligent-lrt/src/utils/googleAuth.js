...
export const useGoogleAuth = () => {
  // Use the correct Expo owner from app.json (owner: "jp-69")
  // This MUST match what's in Google Cloud Console
  const redirectUri = 'https://auth.expo.io/@jp-69/intelligent-lrt-management';
...
export const getGoogleUserInfo = async (response) => {
  try {
    ...
    if (superAdminEmails.includes(userData.email.toLowerCase())) {
      userData.role = 'superadmin';
      console.log('⭐ Super Admin access granted');
    } else if (adminEmails.includes(userData.email.toLowerCase())) {
      userData.role = 'admin';
